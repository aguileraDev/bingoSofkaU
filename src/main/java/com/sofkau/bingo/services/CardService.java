...
public class CardService {
    ...
    @Transactional
    public CardDto createCard(Integer id, CreateCardDto createCardDto) {
        Optional<Game> game;
        Optional<Card> card;
        try {
            game = Optional.of(gameRepository.findByIdAndIsActiveTrue(Long.valueOf(id)).orElseThrow());
    ...
    public List<Integer> generateColumn(int numberMin, int numberMax) {
        ...
        while (values.size() < 5) {
            int temp = (int) (Math.random() * (numberMax - numberMin + 1)) + numberMin; // Corrige el rango
            values.add(temp);
        }
